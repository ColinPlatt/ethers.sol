...
library libBrowserProvider {
    ...
    function connectWallet() internal pure returns (string memory) {
        return
        "const connectWallet=async()=>{if(void 0!==window.ethereum){let e=window.ethereum;if(window.ethereum.providers&&window.ethereum.providers.length){let t=window.ethereum.providers.map(async t=>{t.isMetaMask&&(e=t)});await Promise.all(t)}return e}};";
    }
    ...
    function checkAndChangeChain() internal pure returns (string memory) {
        return
        'const checkAndChangeChain=async(a,e)=>{let t=await a.request({method:"eth_chainId"});if(t!==e)try{await a.request({method:"wallet_switchEthereumChain",params:[{chainId:e}]})}catch(h){if(4902===h.code)try{await a.request({method:"wallet_addEthereumChain",params:[{chainId:e},]})}catch(r){console.error(r)}console.error(h)}};';
    }

    function ethereum_request(string memory _request) internal pure returns (string memory) {
        return string.concat("ethereum.request({", _request, "})");
    }
    ...
    function wallet_addEthereumChain(
        string memory _chainId,
        string memory _chainName,
        NativeCurrency memory nativeCurrency,
        string memory _rpcUrls,
        string memory _blockExplorerUrls
    ) internal pure returns (string memory) {
        return libJsonRPCProvider._request(
    ...
    function wallet_switchEthereumChain(string memory _chainId) internal pure returns (string memory) {
        return libJsonRPCProvider._request("wallet_switchEthereumChain", string.concat('{"chainId":"', _chainId, '"}'));
    }

    function wallet_requestPermissions() internal pure returns (string memory) {
        return libJsonRPCProvider._request("wallet_requestPermissions", string.concat('{"eth_accounts":{}}'));
    }
    ...
    function wallet_requestPermissions(Key[] memory _Keys) internal pure returns (string memory) {
        ...
        return libJsonRPCProvider._request(
            "wallet_requestPermissions", string.concat('{"eth_accounts":{', _KeysString, "}}")
        );
    }

    function wallet_getPermissions() internal pure returns (string memory) {
        return libJsonRPCProvider._request("wallet_getPermissions");
    }

    function wallet_registerOnboarding() internal pure returns (string memory) {
        return libJsonRPCProvider._request("wallet_registerOnboarding");
    }

    function wallet_watchAsset(address _address, string memory _symbol, string memory _decimals, string memory _image)
        internal
        pure
    ...
    function eth_decrypt(string memory _encryptedMessage, address _address) internal pure returns (string memory) {
        return libJsonRPCProvider._request(
            "eth_decrypt", string.concat('"', _encryptedMessage, '","', _address.toHexString(), '"')
        );
    }

    function eth_getEncryptioninternalKey(address _address) internal pure returns (string memory) {
        return
            libJsonRPCProvider._request("eth_getEncryptioninternalKey", string.concat('"', _address.toHexString(), '"'));
    }

    function eth_accounts() internal pure returns (string memory) {
        return libJsonRPCProvider._request("eth_accounts");
    }

    function eth_signTypedData_v4(address _address, string memory _typedData) internal pure returns (string memory) {
        return libJsonRPCProvider._request(
            "eth_signTypedData_v4", string.concat('"', _address.toHexString(), '",', _typedData)
        );
    }
}
