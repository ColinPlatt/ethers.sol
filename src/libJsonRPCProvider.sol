...
library libJsonRPCProvider {
    ...
    function personal_sign(bytes memory _challenge, address _address) internal pure returns (string memory) {
        return
            _request("personal_sign", string.concat('"', _challenge.toHexString(), '","', _address.toHexString(), '"'));
    }
    ...
    function eth_getBlockByHash(bytes32 _blockHash, bool _hydratedTransactions) internal pure returns (string memory) {
        return _request(
            "eth_getBlockByHash",
            string.concat('"', uint256(_blockHash).toHexString(), '",', _hydratedTransactions ? "true" : "false")
        );
    }

    //eth_getBlockByNumber
    function eth_getBlockByNumber(uint256 _blockNumber, bool _hydratedTransactions)
        internal
        pure
        returns (string memory)
    {
        return _request(
            "eth_getBlockByNumber",
            string.concat('"', _blockNumber.toHexString(), '",', _hydratedTransactions ? "true" : "false")
        );
    }
    //@todo by block tag
    ...
    function eth_getBlockTransactionCountByHash(bytes32 _blockHash) internal pure returns (string memory) {
        return
            _request("eth_getBlockTransactionCountByHash", string.concat('"', uint256(_blockHash).toHexString(), '"'));
    }
    ...
    function eth_getUncleByBlockHash(bytes32 _blockHash, uint256 _uncleIndex) internal pure returns (string memory) {
        return _request(
            "eth_getUncleByBlockHash",
            string.concat('"', uint256(_blockHash).toHexString(), '","', _uncleIndex.toHexString(), '"')
        );
    }
    ...
    //eth_estimateGas
    function eth_estimateGas(Transaction memory _transaction, uint256 _blockNumber)
        internal
        pure
        returns (string memory)
    {
    ...
    function eth_estimateGas(Transaction memory _transaction, blockTag _blockTag)
        internal
        pure
        returns (string memory)
    {
    ...
    //eth_feeHistory
    function eth_feeHistory(uint256 _blockCount, uint256 _newestBlock, uint256[] memory _rewardPercentiles)
        internal
        pure
        returns (string memory)
    {
        ...
        return _request(
            "eth_feeHistory",
            string.concat(
                "[", _blockCount.toHexString(), ",", _newestBlock.toHexString(), ",", _rewardPercentilesString, "]"
            )
        );
    }

    //eth_newFilter
    function eth_newFilter(bytes32 _fromBlock, bytes32 _toBlock, address _address)
        internal
        pure
        returns (string memory)
    {
        return _request(
            ...
            string.concat(
                '{"fromBlock":"',
                uint256(_fromBlock).toHexString(),
                '","toBlock":"',
                uint256(_toBlock).toHexString(),
                '","address":"',
    ...
    //eth_getLogs
    function eth_getLogs(bytes32 _fromBlock, bytes32 _toBlock, address _address)
        internal
        pure
        returns (string memory)
    {
    ...
    //eth_submitWork
    function eth_submitWork(bytes8 _nonce, bytes32 _powHash, bytes32 _mixDigest)
        internal
        pure
        returns (string memory)
    {
        return _request(
            ...
            string.concat(
                '"',
                abi.encodePacked(_nonce).toHexString(),
                '","',
                uint256(_powHash).toHexString(),
                '","',
                uint256(_mixDigest).toHexString(),
                '"'
            )
        );
    }
    ...
    function eth_submitHashrate(bytes32 _hashrate, bytes32 _id) internal pure returns (string memory) {
        return _request(
            "eth_submitHashrate",
            string.concat('"', uint256(_hashrate).toHexString(), '","', uint256(_id).toHexString(), '"')
        );
    }
    ...
    function eth_getBalance(address _address, uint256 _blockNumber) internal pure returns (string memory) {
        return _request(
            "eth_getBalance", string.concat('"', _address.toHexString(), '","', _blockNumber.toHexString(), '"')
        );
    }
    //@todo by block tag

    //eth_getStorageAt
    function eth_getStorageAt(address _address, uint256 _position, uint256 _blockNumber)
        internal
        pure
        returns (string memory)
    {
        return _request(
            "eth_getStorageAt",
            string.concat(
                '"', _address.toHexString(), '","', _position.toHexString(), '","', _blockNumber.toHexString(), '"'
            )
        );
    }
    //@todo by block tag
    ...
    function eth_getTransactionCount(address _address, uint256 _blockNumber) internal pure returns (string memory) {
        return _request(
            "eth_getTransactionCount",
            string.concat('"', _address.toHexString(), '","', _blockNumber.toHexString(), '"')
        );
    }
    //@todo by block tag
    ...
    function eth_getCode(address _address, uint256 _blockNumber) internal pure returns (string memory) {
        return
            _request("eth_getCode", string.concat('"', _address.toHexString(), '","', _blockNumber.toHexString(), '"'));
    }
    //@todo by block tag

    //eth_getProof
    function eth_getProof(address _address, uint256[] memory _storageKeys, uint256 _blockNumber)
        internal
        pure
        returns (string memory)
    {
    ...
    function eth_getTransactionByBlockHashAndIndex(bytes32 _blockHash, uint256 _index)
        internal
        pure
        returns (string memory)
    {
        return _request(
            "eth_getTransactionByBlockHashAndIndex",
            string.concat('"', uint256(_blockHash).toHexString(), '","', _index.toHexString(), '"')
        );
    }

    //eth_getTransactionByBlockNumberAndIndex
    function eth_getTransactionByBlockNumberAndIndex(uint256 _blockNumber, uint256 _index)
        internal
        pure
        returns (string memory)
    {
        return _request(
            "eth_getTransactionByBlockNumberAndIndex",
            string.concat('"', _blockNumber.toHexString(), '","', _index.toHexString(), '"')
        );
    }
    //@todo by block tag
